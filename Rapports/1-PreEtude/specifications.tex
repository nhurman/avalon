...
\subsection{Données}
	...
	Unity dispose bien sur de fonctionnalités d'import dans d'autres formats, mais Unity est connu pour être un peu capricieux dans ses interactions avec les .max. 
...
\subsection{Logiciels}
	Au cours de ce projet, nous allons devoir travailler avec plusieurs environnements logiciels; ceux-ci entrent dans deux grandes catégories : les modeleurs 3D, qui permettront d'effectuer de légères modifications au modèle 3D ou de modéliser un téléphone par exemple et les bibliothèques/environnements de développement qui nous permettront de réaliser la partie interactions du sujet.
	...
	\subsubsection{3DSmax}
		\noindent\begin{minipage}{0.3\textwidth}
			...
			3DSmax, le célèbre modeleur 3D d’Autodesk n’est plus à présenter; aujourd’hui encore considéré comme la référence en matière de modélisation 3D, et ce depuis plus de 10 ans, il est grandement utilisé dans l’industrie vidéoludique et filmographique.
	...
	\subsubsection{Blender}
		\noindent\begin{minipage}{0.3\textwidth}
			...
			\hfill
			\begin{minipage}{0.6\textwidth}\raggedleft
			Blender est le modeleur 3D libre le plus avancé. Le projet Blender fut lancé en 1995 et était initialement un logiciel propriétaire développé en interne par Neo Geo et Not a Number Technologies.
			...
			Blender est disponible sous Windows, Mac, Linux et BSD et inclut toutes les fonctionnalités classiques d'un logiciel de se type, mais inclut aussi des fonctionnalités d'extension via des scripts Python ainsi qu'un moteur de jeu.
	...
	\subsubsection{MiddleVR}
		\noindent\begin{minipage}{0.3\textwidth}
			...
			\hfill
			\begin{minipage}{0.6\textwidth}\raggedleft
			MiddleVR est un plugin compatible avec Unity qui s'appuie sur  VRPN. Développé par I’m in VR (PARIS, France) permettant de gérer les interactions entre l’utilisateur et son environnement, et spécialement conçu pour les environnements en réalité virtuelle. 
	...
	\subsubsection{\#Five}
		\#Five est une bibliothèque pour Unity développée en interne au sein de l’Irisa. \#Five ajoute plusieurs couches, en particulier une couche de gestion de relations entre les objets (Une interaction avec un objet A effectue une action sur un objet B).
		Ces relations permettent des scénarios de haut niveau, avec un ordre relatif des interactions entre les étapes. Par exemple, pour démonter une culasse, il faut dévisser les boulons; l'ordre dans lequel on dévisse les dis boulons importe peu.
...
Nous disposons de différents outils pour immerger l'utilisateur au cœur de l'environnement virtuel : 
\\
...
% même les profs disent que c'est une m..de cette chose.
...
%also, on peut s'y mettre à plusieurs ?
\\
...
% et par l'immersion procurée, car on ne voit rien d'autre ? (pas de coup d'oeil par dessus les lunettes, vison à 360° ...)
\\
...
\end{itemize}
% lieu de la salle + possesseur + plus grande salle d'europe ?
...
\\
%non, ce n'est pas le plus intéressant. Pourrais tu modérer ? du type "qui se rapproche le plus de ce que l'on imagine en RV", etc ?
La Kinect développée par Microsoft, est le périphérique le plus intéressant à exploiter dans le cadre de la réalité virtuelle. Elle permet en effet de détecter la présence de 6 personnes et de suivre les mouvements de deux utilisateurs actifs grâce à ses lentilles placées sur un socle motorisé. Sa portée est comprise entre 1,2m et 3,5m. L'intérêt principal de la Kinect est que l'utilisateur puisse se déplacer librement dans une pièce sans avoir à manipuler une quelconque manette.
\\

\textbf{WiiMote, Nunchuck et WiiMotionPlusInside}
\\
% moins pratique, sauf pour pointer ! Also, tu dispose de boutons dessus, pour par exemple marcher, accéder au menus ...
...
\\
%il a pas de retour de force en fait, dixit ronan. Pourquoi ne pas parler d'un matériel générique à la place ?
Avec ses commandes avancées et sa gouverne à manche rotatif, ce joystick à retour de force stable et précis est prévu pour être connecté à un ordinateur et est utilisé pour des jeux de combat aérien acrobatique. Il est composé d'une gouverne à manche rotatif et de nombreux boutons.
\\
